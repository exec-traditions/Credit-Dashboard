...
/**
 * Update a card's points program name and/or estimated value per point.
 * PATCH /api/points/[id]/settings   ([id] = card_points.id)
...
export async function PATCH(
  ...
  const { error } = await db.from('card_points').update(update).eq('id', id)
